...
import { EmployeeAppointmentRow } from './EmployeeAppointmentRow';
...
export const EmployeeAppointmentList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        if (!employeeId) {
          setError('Not logged in as an employee. Please login first.');
          setLoading(false);
          return;
        }
        ...
        // Filter appointments for this employee only
        const employeeAppointments = appointmentsData.filter(
          apt => apt.employeeId === parseInt(employeeId)
        );
  ...
  if (appointments.length === 0) {
    return (
      <div className="flex justify-center items-center h-64 text-white/70">
        <span className="text-lg italic">No appointments assigned to you</span>
      </div>
    );
  }
